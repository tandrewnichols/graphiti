...
module.exports = class Graph extends EventEmitter {
  ...
  register({ nodes, relationships }) {
    ...
    return this.registerNodes(nodes)
      .then(() => this.registerRelationshipTypes(relationships))
  }
   ...
   *  
   */
  registerNodes(patterns) {
    // Lookup the modesl
    return this._register(patterns).then(nodes => {
      ...
      nodes.forEach(node => this[ node.name ] = node)
      return nodes
    })
  }
